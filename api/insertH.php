<?php
	...
	if(!isset($data->rut)|| !isset($data->N_pacientes)|| !isset($data->E_mandante)|| !isset($data->E_convenio)|| !isset($data->f_ingreso)|| !isset($data->consentimiento)|| !isset($data->f_recepcion)):
	...
	exit;
	elseif (empty(trim($data->rut))||empty(trim($data->N_pacientes))||empty(trim($data->E_mandante))||empty(trim($data->E_convenio))||empty(trim($data->f_ingreso))||empty(trim($data->consentimiento))||empty(trim($data->f_recepcion))):
...
try{
    ...
    $f_recepcion =  htmlspecialchars(trim($data->f_recepcion));
	...
	$query = "INSERT INTO `historial`(
        ...
        $stmt->bindValue(':f_recepcion', $f_recepcion, PDO::PARAM_STR);
